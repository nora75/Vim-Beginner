...
" コメントには説明と、その後にその設定を無効化する、または
" (乂･д･´)反対!の動作をする設定が書かれています
" このファイル内に個人的な設定を書かないで下さい(Ex:プラグインの設定等)
" このファイル内とは異なる値を設定したい場合、
" このファイル**読み込み後**に各自で設定して下さい
" エンコードの設定は危険なため、このファイルには入ってません、
" 各自で設定して下さい、また、このファイルはutf-8で書かれています
" 各自で設定する場合vimrcの一番上に書いて下さい
